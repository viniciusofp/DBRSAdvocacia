...
http.createServer(function(request, response) {
    ...
    if(request.url === "/index"){
        sendFileContent(response, "build/index.html", "text/html");
    }
    else if(request.url === "/"){
    ...
    else if(/^\/[a-zA-Z0-9\/]*.jpg$/.test(request.url.toString()) || /^\/[a-zA-Z0-9\/]*.png$/.test(request.url.toString())){
        sendFileContent(response, request.url.toString().substring(1), "image");
    }
